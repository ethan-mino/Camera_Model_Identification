...
import re
import cv2
from extract_exif import get_exif
...
def get_last_checkpoint(checkpoint_dir) : 
    ...
    checkpoint_name = sorted(filename_list, reverse = True)[0]
...
def get_img_size(file_path) : 
    try : 
        ...
        height = int(exif["ExifImageHeight"])
    except : 
...
def label_distribution(labels, desc = "") :
    sorted_label = sorted(labels)
    distribution = {}

    print(desc, ": ")
    for key, group in groupby(sorted_label) : 
        distribution[key] = len(list(group))

    return distribution

def get_datasets_path(dataset_path, max_per_class = None, desc = "") : 
    X, y = [], []
    ...
    for model_name in os.listdir(dataset_path) : 
        ...
        if max_per_class is not None : 
            paths = get_all_files(model_path)[ : max_per_class]
        else : 
            paths = get_all_files(model_path)

        X = X + paths
        y = y + ([model_name] * len(paths))

    pprint(label_distribution(y, desc =  desc + " (max_per_class : {})".format(max_per_class)))
    print(desc, " : ", np.unique(y))
    return (X, y)